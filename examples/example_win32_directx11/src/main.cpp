...
static ID3D11ShaderResourceView* g_ravenLogo = nullptr;
static ID3D11ShaderResourceView* g_ravenBanner = nullptr;
...
#include "app/account_state.inc"
#include "ravenxd_rpc.h"

int main(int, char**)
{
    HANDLE hOneInstance = ::CreateMutexW(nullptr, TRUE, L"Local\\RavenXDLauncherSingleInstance");
    if (hOneInstance && ::GetLastError() == ERROR_ALREADY_EXISTS) {
        if (HWND prev = ::FindWindowW(L"RavenXDLauncher", nullptr)) ::SetForegroundWindow(prev);
        return 0;
    }
    ...
    LoadOrCreateConfig();
    WNDCLASSEXW wc = { sizeof(wc), CS_CLASSDC, WndProc, 0L, 0L, GetModuleHandle(nullptr), nullptr, nullptr, nullptr, nullptr, L"RavenXDLauncher", nullptr };
    ::RegisterClassExW(&wc);
    ...
    HWND hwnd = ::CreateWindowExW(WS_EX_LAYERED, wc.lpszClassName, L"RavenXD Launcher",
    ...
    RavenXD_RPC::Initialize();
    RavenXD_RPC::UpdatePresence("RavenXD Launcher", "Minecraft 1.8.9");
    LoadPhotos(g_pd3dDevice);
    {
        std::string logoPath = AssetPath("assets\\RavenXD-logo.png", nullptr);
        std::wstring wlogo(logoPath.begin(), logoPath.end());
        g_ravenLogo = LoadWICTexture(g_pd3dDevice, wlogo.c_str());

        std::string bannerPath = AssetPath("assets\\RavenXD-banner.png", nullptr);
        std::wstring wbanner(bannerPath.begin(), bannerPath.end());
        g_ravenBanner = LoadWICTexture(g_pd3dDevice, wbanner.c_str());
    }
...
#include "app/config_apply.inc"
    ...
    LARGE_INTEGER lastBlurTick = {}, lastFrameTick = {}, lastRpcTick = {};
    bool done = false;
    while (!done)
    {
        MSG msg;
        ...
        {
            LARGE_INTEGER rpcNow; ::QueryPerformanceCounter(&rpcNow);
            double rpcElapsed = lastRpcTick.QuadPart ? (double)(rpcNow.QuadPart - lastRpcTick.QuadPart) / (double)s_qpf.QuadPart : 999.0;
            if (rpcElapsed >= 3.0) {
                lastRpcTick = rpcNow;
                const int setupStateNow = RavenLauncher::setupState.load();
                const bool forgeOk = RavenLauncher::ForgeInstalled();
                const bool modOk = RavenLauncher::ModInstalled();

                const char* details = "Minecraft 1.8.9";
                const char* stateText = ravenAccounts.empty() ? "No profile selected" : ravenAccounts[ravenSelectedAccount].c_str();

                if (setupStateNow == RavenLauncher::Working) {
                    details = forgeOk ? "Installing RavenXD" : "Installing Forge";
                    stateText = "RavenXD Launcher";
                } else if (setupStateNow == RavenLauncher::Failed) {
                    details = "Setup failed";
                    stateText = "RavenXD Launcher";
                } else if (forgeOk && modOk) {
                    details = (page == 1) ? "Managing Minecraft accounts" : "Ready to launch";
                } else {
                    details = "RavenXD Launcher";
                    stateText = "Setup required";
                }

                if (RavenXD_RPC::IsConnected()) RavenXD_RPC::UpdatePresence(details, stateText);
                RavenXD_RPC::UpdateAutoCycle();
            }
        }
        ...
        if (Glass::BeginCard("dash", Glass::Material::Regular, ImVec2(dashW, dashH))) {
            ...
            Glass::Icon navIcons[] = { Glass::Icon::Gamepad, Glass::Icon::Person };
            const char* navLabels[] = { "Minecraft", "Accounts" };
            ImU32 navTints[] = { IM_COL32(10,132,255,255), IM_COL32(94,92,230,255) };
            const int navCount = 2;
            { float bs = 40.0f * uiScale;
              if (g_ravenLogo) {
                  wdl->AddImageRounded((ImTextureID)(intptr_t)g_ravenLogo,
                      ImVec2(o.x, o.y), ImVec2(o.x + bs, o.y + bs),
                      ImVec2(0,0), ImVec2(1,1), IM_COL32(255,255,255,255), 12.0f*uiScale);
              } else {
              ...
              wdl->AddText(font_semibold, 22.0f*uiScale, ImVec2(o.x + bs + 12.0f*uiScale, o.y + 8.0f*uiScale), IM_COL32(236,237,243,255), "RavenXD");
              wdl->AddText(font_medium, 11.0f*uiScale, ImVec2(o.x + bs + 13.0f*uiScale, o.y + 31.0f*uiScale), IM_COL32(160,180,220,220), "MINECRAFT 1.8.9"); }
            ...
            Glass::DrawActionSheet();
            {
              ...
              if (ncmds == 0) {
                  const char* pages[2] = { "Minecraft", "Accounts" };
                  for (int i=0;i<2;i++) cmds[ncmds++] = pages[i];
            ...
            if (authA > 0.5f && !hideChrome) {
                ...
                Glass::DynamicIsland(Glass::Icon::Bolt, "RavenXD", g_recording ? "REC" : "Ready",
        ...
        if (!welcomed && appClock > 1.0f) {
            Glass::Notify(Glass::Icon::CheckCircle, IM_COL32(52,209,88,255), "RavenXD",
                          "Launcher ready", "Forge 1.8.9 and RavenXD mod are available from the launcher.");
            welcomed = true;
        }
    ...
    RavenXD_RPC::Shutdown();
    RavenLauncher::Shutdown();
    if (g_ravenBanner) { g_ravenBanner->Release(); g_ravenBanner = nullptr; }
    if (g_ravenLogo) { g_ravenLogo->Release(); g_ravenLogo = nullptr; }
    g_backdrop.Shutdown();
